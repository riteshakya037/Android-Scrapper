...
public class AFL_Spread extends AFL {
    public static final Creator<AFL_Spread> CREATOR = new Creator<AFL_Spread>() {
        @Override public AFL_Spread createFromParcel(Parcel in) {
            return new AFL_Spread(in);
        }

        @Override public AFL_Spread[] newArray(int size) {
    ...
    @SuppressWarnings("UnusedParameters") private AFL_Spread(Parcel in) {
        // Empty Block
    }
    ...
    @Override public String getContraryPackageName() {
        return AFL_Total.class.getName();
    }
}
