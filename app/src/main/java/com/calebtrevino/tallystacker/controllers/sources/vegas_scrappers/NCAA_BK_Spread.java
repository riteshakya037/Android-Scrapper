...
public class NCAA_BK_Spread extends NCAA_BK {
    public static final Creator<NCAA_BK_Spread> CREATOR = new Creator<NCAA_BK_Spread>() {
        @Override public NCAA_BK_Spread createFromParcel(Parcel in) {
            return new NCAA_BK_Spread(in);
        }

        @Override public NCAA_BK_Spread[] newArray(int size) {
            return new NCAA_BK_Spread[size];
        }
    };
    private final ScoreType BID_SCORE_TYPE = ScoreType.SPREAD;

    @SuppressWarnings("unused") public NCAA_BK_Spread() {
    }

    @SuppressWarnings("UnusedParameters") private NCAA_BK_Spread(Parcel in) {
        // Empty Block
    }
    ...
    @Override public String getContraryPackageName() {
        return NCAA_BK_Total.class.getName();
    }
}
