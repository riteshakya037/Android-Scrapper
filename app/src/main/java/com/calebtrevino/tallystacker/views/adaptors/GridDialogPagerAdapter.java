...
public class GridDialogPagerAdapter extends FragmentStatePagerAdapter {
    ...
    public View getTabView(int position) {
        ...
        @SuppressLint("InflateParams") View view =
                LayoutInflater.from(mContext).inflate(R.layout.grd_view_dialog_item, null);
        ...
        teamsName.setText(mContext.getString(R.string.team_vs_team,
                mData.get(position).getFirstTeam().getName().equals(DefaultFactory.Team.NAME)
                        ? mData.get(position).getFirstTeam().getCity()
                        : mData.get(position).getFirstTeam().getName(),
                mData.get(position).getSecondTeam().getName().equals(DefaultFactory.Team.NAME)
    ...
    private void setBannerVisibility(long ts1, long ts2, View bannerView) {
        if (ts2 == 0) {
            ...
            cal1.setTime(new DateTime(ts1, Constants.DATE.VEGAS_TIME_ZONE).toDateTime(
                    DateTimeZone.getDefault()).toDate());
            cal2.setTime(new DateTime(ts2, Constants.DATE.VEGAS_TIME_ZONE).toDateTime(
                    DateTimeZone.getDefault()).toDate());
