...
public class GridViewAdapter extends RecyclerView.Adapter<GridViewAdapter.GridViewHolder> {
    ...
    @Override public GridViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.grd_view_item, parent, false);
    ...
    @SuppressWarnings("unused") public void addGames(Game game) {
    ...
    @Override public void onBindViewHolder(GridViewHolder holder, final int position) {
        if (position < data.size()) {
            ...
            holder.bannerView.setVisibility(
                    currentGame.getBannerVisibility() ? View.VISIBLE : View.GONE);
    ...
    @SuppressWarnings("unused") public void addGames(List<Game> gameList) {
        data.addAll(gameList);
    }

    @SuppressWarnings("unused") public void setData(List<Game> gameList) {
        data = new LinkedList<>();
        data.addAll(gameList);
        this.notifyDataSetChanged();
    }

    @SuppressWarnings("WeakerAccess") class GridViewHolder extends RecyclerView.ViewHolder {

        @BindView(R.id.leagueName) protected TextView leagueName;
        @BindView(R.id.teamsName) protected TextView teamsName;
        @BindView(R.id.gridMarker) protected TextView gridMarker;
        @BindView(R.id.new_banner) protected View bannerView;
        @BindView(R.id.root_view) protected View rootView;
        ...
        protected void setOnClickListener(final int position) {
            rootView.setOnClickListener(new View.OnClickListener() {
                @Override public void onClick(View view) {
                    GridViewDialog.newInstance(new ArrayList<>(data), position)
        ...
        protected void setGridMarker(int position) {
            if (data.get(position).getGameAddDate() == new DateTime(
                ...
                switch (data.get(position).getPreviousGridStatus()) {
                    case NEUTRAL:
                        gridMarker.setBackgroundColor(
                                ContextCompat.getColor(mContext, R.color.colorDraw));
                        break;
                    case NEGATIVE:
                        gridMarker.setBackgroundColor(
                                ContextCompat.getColor(mContext, R.color.colorError));
                        break;
                    case DRAW:
                        gridMarker.setBackgroundColor(
                                ContextCompat.getColor(mContext, R.color.colorDraw));
