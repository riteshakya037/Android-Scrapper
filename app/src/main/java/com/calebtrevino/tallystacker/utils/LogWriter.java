...
public class LogWriter {
    ...
    public static void writeLog(Context baseContext) {
        ...
        MultiProcessPreference.getDefaultSharedPreferences()
                .edit()
                .putString(LOG_FILE_LOCATION, file.getAbsolutePath())
                .commit();
    }

    @NonNull private static FileWriter writeOutput(PackageInfo info, String model, File file,
    ...
    @NonNull private static InputStreamReader getInputStreamReader() throws IOException {
    ...
    @NonNull private static String getModel() {
        String model = Build.MODEL;
        if (!model.startsWith(Build.MANUFACTURER)) model = Build.MANUFACTURER + " " + model;
        return model;
    }

    @Nullable private static PackageInfo getPackageInfo(Context baseContext) {
    ...
    public static void sendLog(FragmentActivity context) {
        File file = new File(MultiProcessPreference.getDefaultSharedPreferences()
                .getString(LOG_FILE_LOCATION, ""));
        if (!file.exists()) return;
        Uri contentUri =
                FileProvider.getUriForFile(context, "com.calebtrevino.tallystacker.file_provider",
                        file);
        if (contentUri != null) {
            ...
            context.startActivityForResult(Intent.createChooser(shareIntent,
                    context.getResources().getText(R.string.send_to)), LOG_RESULT);

            new Handler().postDelayed(new Runnable() {
                @Override public void run() {
                    MultiProcessPreference.getDefaultSharedPreferences()
                            .edit()
                            .putString(LOG_FILE_LOCATION, "")
                            .commit();
